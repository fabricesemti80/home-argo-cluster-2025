...
**Date**: March 18, 2026  
**Status**: 🔴 Critical issues found - OOM risk on Prometheus & Jellyfin
...
| **jellyfin** | NO resource config | 386Mi | ❌ None | ❌ None | 🔴 OOM |
...
2. **jellyfin** (media/jellyfin)
   - Current: 386Mi, **No Config at all**
   - Fix: Request: 256Mi, Limit: 512Mi (can spike during transcoding)

3. **grafana** (monitoring/kube-prometheus-stack)
   - Current: 309Mi, **No Config**
   - Fix: Request: 256Mi, Limit: 512Mi

### 🟡 HIGH (System-Critical or Unstable Probes)

4. **argo-cd components** (argo-system/argo-cd)
...
10. **cloudflare-dns** (network/cloudflare-dns)
    - Current: 31Mi, **No config**
    - Fix: Request: 32Mi, Limit: 256Mi

11. **k8s-gateway** (network/k8s-gateway)
    - Current: 18Mi, **No config**
    - Fix: Request: 32Mi, Limit: 128Mi

12. **doppler-operator** (doppler-operator-system/doppler-operator)
    - Current: 22Mi, **No config**
    - Fix: Request: 32Mi, Limit: 256Mi

13. **reloader** (kube-system/reloader)
    - Current: 31Mi, **No config**
    - Fix: Request: 32Mi, Limit: 128Mi

14. **cert-manager** (cert-manager/cert-manager)
    - Current: 70/45/21Mi, **No config**
    - Fix: Add resource limits

15. **echo** (default/echo)
...
2. Jellyfin: Add 256Mi request, 512Mi limit
3. Grafana: Add 256Mi request, 512Mi limit
4. Cilium: Add 200Mi request, 512Mi limit (DaemonSet)

### Phase 2: System-Critical (Next)
5. Argo-CD: All components - add resource limits
6. CoreDNS: Add 64Mi request, 256Mi limit
7. metrics-server: Add 64Mi request, 256Mi limit
8. spegel: Add 32Mi request, 256Mi limit

### Phase 3: Networking (Day 2)
9. cloudflare-tunnel: Fix probe timings + add memory request
10. cloudflare-dns: Add resource limits
11. k8s-gateway: Add resource limits
12. envoy-gateway: Add resource limits

### Phase 4: Misc (Day 2)
13. doppler-operator: Add resource limits
14. reloader: Add resource limits
15. cert-manager: Add resource limits
...
| Jellyfin crash on transcoding | HIGH | Service unavailable | Limit: 512Mi, Request: 256Mi |
...
kubernetes/apps/media/jellyfin/values.yaml
